...
%dbdate2datenum convert string format dates returned by a database to
%Matlab datenums
%
%   Syntax:
%       [dn, millis] = dbdate2datenum(dbdate)
% dndate can ne a single char array or string, or can be a cell array of
% char arrays or of string, e.g. a columns of a table returned from a
% database query. 
...
if iscell(dbdate)
    ...
    for i = 1:n
        [aDate, ms] = pgmatlab.utils.dbdate2datenum(dbdate{i});
        dn(i) = aDate;
        millis(i) = ms;
    end
    return;
