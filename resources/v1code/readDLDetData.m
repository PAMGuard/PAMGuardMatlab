function [data, error] = readDLDetData(fid, fileInfo, data)
...
%         disp(['data.nChan' num2str(data.nChan )])
        data.nSamps = fread(fid, 1, 'int32');
%          disp(['data.nChan' num2str(data.nSamps )])
        data.scale = 1/fread(fid, 1, 'float');
%          disp(['data.scale' num2str(data.scale )])
